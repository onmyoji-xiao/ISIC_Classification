...
from dataset_l import MyDataset_G
...
image_datasets = {x: MyDataset_G(csv_path=path + x + ".csv",
                                 image_path=path + x,
                                 lbp_path=path+"local/lbp_" + x + ".csv",
...
for epoch in range(num_epochs):
    ...
    for phase in ['train', 'test']:
        ...
        class1 = 0
        class2 = 0
        for imgs, features, labels in dataloders[phase]:
            count_batch += 1

            score = (torch.zeros(batch_size, 3)).to(device)

            input1 = imgs[0].to(device)
            ll = labels[0].to(device)

            i = 0
            while i < 4:
                optimizer.zero_grad()
                input0 = imgs[i].to(device)
                feature0 = features[i].to(device)
                output0 = model(input0)

                x = torch.cat((output0, feature0), 1)
                y = model2(x)
                score = score + y
            ...
            running_corrects += torch.sum(preds == ll.data)
            counti += input1.size(0)
            if phase == 'test':
                for k in range(input1.size(0)):
                    if ll.data[k] == 0 and preds[k] == 0:
                        class0 += 1
                    if ll.data[k] == 1 and preds[k] == 1:
                        class1 += 1
                    if ll.data[k] == 2 and preds[k] == 2:
                        class2 += 1
        ...
        if phase == 'train':
            if (epoch + 1) % 50 == 0:
                ...
                torch.save(model, '/home/xf/model/densenet121-lbp-l_epoch{}.pkl'.format(epoch + 1))
        # deep copy the model
        if phase == 'test':
            if epoch_acc > best_acc:
                ...
                best2 = class2
                best_acc = epoch_acc
                torch.save(model, "/home/xf/bestmodel/densenet121-l-lbp-sal.pkl")
                torch.save(model2, "/home/xf/bestmodel/dense-dnn-l-lbp-sal.pkl")
